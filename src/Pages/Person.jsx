...
export default function person() {
  ...
  const [currentPage, setCurrentPage] = useState(
    parseInt(localStorage.getItem('currentPagePerson'), 10) || 1
  );
  // تعريف الحالة لتحديد ما إذا كان يتم تحميل البيانات
  const [loading, setLoading] = useState(true);
  // تعريف الحالة لتخزين استعلام البحث من localStorage أو قيمة فارغة
  const [searchQuery, setSearchQuery] = useState(localStorage.getItem('searchQueryPerson') || '');
  ...
  async function getPerson(page) {
    ...
    try {
      const requests = [
        axios.get(`https://api.themoviedb.org/3/person/popular?api_key=f597813c136fdbe4ff8e3e2976da14ad&language=en-US&page=${page}`),
        axios.get(`https://api.themoviedb.org/3/person/popular?api_key=f597813c136fdbe4ff8e3e2976da14ad&language=en-US&page=${page + 1}`)
      ];
      ...
      setPerson(results.slice(0, 24)); // أخذ أول 24 نتيجة فقط
  ...
  async function searchPerson(query) {
    ...
    try {
      const requests = [
        axios.get(`https://api.themoviedb.org/3/search/person?api_key=f597813c136fdbe4ff8e3e2976da14ad&language=en-US&include_adult=false&query=${query}&page=1`),
        axios.get(`https://api.themoviedb.org/3/search/person?api_key=f597813c136fdbe4ff8e3e2976da14ad&language=en-US&include_adult=false&query=${query}&page=2`)
      ];
      const responses = await Promise.all(requests);
      const results = responses.flatMap(response => response.data.results)
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim() !== '') {
      ...
      getPerson(currentPage); // جلب الأفلام إذا لم يكن هناك استعلام
      setDisablePagination(false); // تمكين الترقيم في حالة عدم البحث
    }
  }, [currentPage, searchQuery]);
  // لحفظ رقم الصفحة
  useEffect(() => {
    localStorage.setItem('currentPagePerson', currentPage);
  }, [currentPage]);
  ...
  return (
    <>
      {loading ? <div className="Layout"> <MoonLoader size={80} color={"#fe1f20"} loading={loading} /> </div> : <div className='container'>
          ...
          <div className='search-input-container'>
            <input
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('searchQueryPerson', value); // يُخزن قيمة البحث في localStorage
                ...
                if (value === '') {
                  localStorage.removeItem('searchQueryPerson'); // يُزيل مفتاح 'searchQuery' من localStorage إذا كانت القيمة فارغة
                }
              }}
        ...
        <div className='content-media'>
          <div className='row gy-3'>
            {loading ? (
              <div className="Layout"> <MoonLoader size={80} color={"#fe1f20"} loading={loading} /> </div>)
              // خاص بنتيجة البحث
                    ...
                    <div key={index} className='col-md-2 media-info'>
                      <Link to={`/itemsdetails/${item.id}/${mediaType}`}>
                        <div className='image-link'>
                          <LazyLoadImage
                            src={item.profile_path ? 'https://image.tmdb.org/t/p/w500' + item.profile_path : defaultPosterPath}
                            alt={item.name}
                            effect='blur'
                          />
